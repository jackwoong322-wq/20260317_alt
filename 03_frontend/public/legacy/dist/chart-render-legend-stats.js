...
import { findManifestCoin, getCycleDisplayName, getCycleStatus, } from './chart-lazy-load.js';
// ── Legend ────────────────────────────────────────────
...
export function updateStats() {
    ...
    if (!coinData && !manifestCoin)
        return;
    let html = '';
    const cycles = manifestCoin?.cycles || coinData?.cycles || [];
    cycles.forEach((manifestCycle) => {
        const cycleNumber = Number(manifestCycle.cycle_number);
        if (!chartState.activeCycles.has(cycleNumber))
            return;
        const cycle = (coinData?.cycles || []).find((item) => Number(item.cycle_number) === cycleNumber);
        ...
        if (status !== 'loaded') {
            html += `
      <div class="stat-item">
        <div class="stat-label">${getCycleDisplayName(chartState.selectedCoins[0], cycleNumber).toUpperCase()}</div>
