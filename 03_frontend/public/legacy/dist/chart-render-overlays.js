...
function clampCoordToVisible(timeScale, day, overlayWidth) {
    ...
    if (typeof coordToTime !== 'function')
        return 0;
...
function safeSubBoxNumber(value) {
    const num = Number(value);
    return Number.isFinite(num) ? num : null;
}
function formatSubBoxRange(box) {
...
export function renderSubBoxMarks(subBoxes, candidates, timeScale, series, coinId, coinSymbol, cycleNumber) {
    if (!series || !timeScale) {
    ...
    items.forEach(({ box, candidate }) => {
        ...
        const upper = safeSubBoxNumber(box.upper);
        if (startX == null || endX == null || upper == null)
            return;
        ...
        if (x1 === null)
            x1 = clampCoordToVisible(timeScale, startX, overlayWidth);
        if (x2 === null)
            x2 = clampCoordToVisible(timeScale, endX, overlayWidth);
...
export function renderBoxMarks(zones, cycleLowIdx, cycleData, timeScale, series, coinId, coinSymbol, cycleNumber, cycleRef) {
    if (!series || !timeScale) {
    ...
    const placedLowRects = [];
    const placedHighRects = [];
    ...
    function getLabelRectInOverlay(el) {
    ...
    function nudgeUntilNoOverlap(el, placed, nudgeX, nudgeY) {
    ...
    if (chartState.showBoxZone && zones.length > 0) {
        zones.forEach((z, zi) => {
            ...
            const hiDayDisp = z.hiDay != null && z.hiDay >= z.startX && z.hiDay <= z.endX
                ? z.hiDay
                : '-';
            const loDayDisp = z.loDay != null && z.loDay >= z.startX && z.loDay <= z.endX
                ? z.loDay
                : '-';
            const hiDateDisp = hiDayDisp !== '-'
                ? cycleData.find((d) => d.x === hiDayDisp)?.date ?? ''
                : '';
            const loDateDisp = loDayDisp !== '-'
...
function renderBearBullLabels(segments, timeScale, series) {
    clearBearBullLabels();
    if (!chartState.showBearBull || segments.length === 0)
        return;
    const overlay = document.getElementById('bearBullOverlay');
    if (!overlay)
        return;
...
export function scheduleRedrawBoxMarks() {
    if (chartState.boxMarksData.length === 0)
        return;
...
function redrawBoxMarks() {
    if (chartState.boxMarksData.length === 0 || !chartState.chart)
        return;
    const overlay = document.getElementById('boxMarksOverlay');
    clearBoxMarks();
    chartState.boxMarksData.forEach((d) => {
        const series = chartState.seriesMap[d.seriesKey];
        if (!series)
            return;
        const ts = chartState.chart.timeScale();
        renderBoxMarks(d.zones, d.cycleLowIdx, d.cycleData, ts, series, d.coinId, d.symbol, d.cycleNumber, d.cycleRef);
    });
}
