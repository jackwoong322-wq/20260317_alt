...
export function getScenarioKey(result) {
    if (!result)
        return 'es';
    const r = (result || '').toUpperCase();
    if (r.includes('_PLUS') || r.includes('CHAIN_PLUS'))
        return 'plus';
    if (r.includes('_MINUS') || r.includes('CHAIN_MINUS'))
        return 'minus';
    return 'es';
}
...
const BTC_COIN_ID = Object.keys(ALL_DATA).find((id) => (ALL_DATA[id]?.symbol || '').toUpperCase() === 'BTC') || null;
// [설계 결정] 단일 state 객체로 전역 let 제거. DI 시 이 객체를 주입.
const _state = {
    selectedCoins: (BTC_COIN_ID ? [BTC_COIN_ID] : []),
    activeCycles: new Set([1, 2, 3, 4, 5]),
    showHighLow: false,
    showBoxZone: true,
    showBearBull: false,
    showPrediction: true,
...
export const chartState = {
    ...
    set chart(v) { _state.chart = v; },
    get seriesMap() { return _state.seriesMap; },
    set seriesMap(v) { _state.seriesMap = v; },
    get seriesMetaMap() { return _state.seriesMetaMap; },
    set seriesMetaMap(v) { _state.seriesMetaMap = v; },
    get boxMarksData() { return _state.boxMarksData; },
    set boxMarksData(v) { _state.boxMarksData = v; },
    get selectedCoins() { return _state.selectedCoins; },
    set selectedCoins(v) { _state.selectedCoins = v; },
    get activeCycles() { return _state.activeCycles; },
    set activeCycles(v) { _state.activeCycles = v; },
    get showHighLow() { return _state.showHighLow; },
    set showHighLow(v) { _state.showHighLow = v; },
    get showBoxZone() { return _state.showBoxZone; },
    set showBoxZone(v) { _state.showBoxZone = v; },
    ...
    get showSubBox() { return _state.showSubBox; },
    set showSubBox(v) { _state.showSubBox = v; },
    get showBB() { return _state.showBB; },
    set showBB(v) { _state.showBB = v; },
 ...
 * ---
 */
export function buildStepSeries(points) {
    if (!points || points.length === 0)
        return [];
    return [...points];
}
// ── Box Zone detection (JS fallback when DB has no data) ─
export function detectBoxZones(data) {
    const zones = [];
    ...
    function bullBoxHiLo(phase2, boxStart, boxEnd) {
        ...
        for (let k = boxStart + 1; k <= boxEnd; k++) {
            if (phase2[k].low < phase2[loIdx].low)
                loIdx = k;
        }
        ...
        for (let k = boxStart; k < loIdx; k++) {
            if (phase2[k].high > phase2[hiIdx].high)
                hiIdx = k;
        }
...
export function detectBearBull(data) {
...
export function getBtcAnchorInfo(cycleNumber) {
    if (!BTC_COIN_ID)
        return null;
    ...
    if (!cyc || !cyc.data || cyc.data.length === 0)
        return null;
    ...
    if (progress >= 0.85)
        ...
        level = 'MID';
    const label = level === 'HIGH' ? 'High Risk' : level === 'MID' ? 'Caution' : 'Normal';
    return { progress, level, label };
}
export function getPhaseBoxStatsForSymbol(symbol, phase) {
    const coinId = Object.keys(ALL_DATA).find((id) => (ALL_DATA[id]?.symbol || '').toUpperCase() === symbol.toUpperCase());
    if (!coinId)
        return null;
    ...
    d.cycles.forEach((c) => {
        ...
        const cnt = zs.filter((z) => z.phase === phase && !z.is_prediction)
            .length;
        if (cnt > 0)
            counts.push(cnt);
    });
