...
- `03_frontend/`: React 18 + Vite 5 app deployed on Vercel
- `02_backend/`: FastAPI + Uvicorn service deployed on Render
- `01_pairUSDT/`: Python scripts for Binance OHLCV collection, cycle analysis, prediction, and legacy chart rendering
- `supabase/functions/`: Supabase Edge Functions using Deno and `supabase-js`
- `.agents/skills/`: Shared project-local agent skills and design workflows
- `reports/`: Project notes, review docs, and improvement plans

## Frontend Rules

Applies to `03_frontend/`.

- Use functional components and hooks only.
- Use `fetch` for API calls.
- Read the backend base URL from `VITE_API_URL`.
- Keep the frontend aligned with the legacy analyzer UI when the user asks for parity with `01_pairUSDT/033_visualizer_html.py`.
...
- Legacy analyzer assets live under `public/legacy/` and should be treated as production assets, not throwaway mocks.
