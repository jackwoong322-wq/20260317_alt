...
from lib.predictor.predict_box_bull import (
    build_bull_chain,
...
from lib.analyzer.db import compute_day_metrics, ensure_analysis_result_columns
...
def _print_btc_prediction_box(
    ...
    _bull_lines = (
        [
            ...
            "| "
            + f"  day {bull_meta['bull_start']}~{bull_meta['bull_end']}  ({bull_meta['pred_dur_bull']}d)".ljust(
                _pad
            )
            + " |",
            "| "
            + f"  hi = {bull_meta['bull_hi']:.2f}%   lo = {bull_meta['bull_lo']:.2f}%   range = {bull_meta['range_bull']:.1f}%".ljust(
                _pad
            )
            + " |",
            "| "
    ...
    _rows = (
        [
            _line,
            "| " + f"BTC 예측 결과 요약  (Cycle {max_cyc})".ljust(_pad) + " |",
            _line,
        ]
        + [
            "| "
            + f"현재 마지막 박스 : #{int(last['box_index'])}  {last['phase']}  day {int(last['start_x'])}~{int(last['end_x'])}".ljust(
                _pad
            )
            + " |"
        ]
        + [
            "| "
            + f"                   hi={float(last['hi']):.2f}%  lo={float(last['lo']):.2f}%".ljust(
                _pad
            )
            + " |",
            _line,
        ...
        + [
            "| "
            + f"[유사 패턴]  {sim_symbol}  Cycle {sim_cycle}  Box #{sim_box}  유사도 {similarity*100:.0f}%".ljust(
                _pad
            )
            + " |",
            _line,
...
def _log_bear_chain_verbose(
    ...
    chain_rows = [
        r
        for r in pred_rows
        if r[1] == str(last["symbol"]) and r[3] == max_cyc and r[7] == "PRED_BEAR_CHAIN"
    ]
    if chain_rows and _verbose:
        log.info("    Bear 체인 박스 (%d개):", len(chain_rows))
        for cr in chain_rows:
            log.info(
                "      chain box#%d  day %d~%d (%dd)  hi=%.2f%%  lo=%.2f%%",
                cr[5],
                cr[8],
                cr[9],
                cr[14],
...
def _log_coin_prediction_verbose(
    ...
    if _verbose:
        ...
        log.info(
            "    phase 확률: P(BULL)=%.3f  P(BEAR)=%.3f  → %s",
            prob_bull,
            prob_bear,
            "BULL" if pred_is_bull else "BEAR",
        )
...
def _predict_one_coin_phase1(
    ...
    (
        ...
        pred_lo_bull,
        pred_dur_bull,
    ) = get_model_predictions(group_models, X_pred, last, reg_key=reg_key)
    ...
    if is_btc_coin:
        ...
        bottom_lo, bottom_day, prob_bear_t, prob_bull_t = calc_bottom_alt(
            bottom_models, group_key, X_pred, last
        )
        # ALT bottom 모델 미학습(샘플<30) 시 폴백: 현재 박스 기준으로 bear chain 진입 허용
        if (
            bottom_lo is None
            and bottom_day is None
            and bottom_models.get(group_key) is None
        ):
            ref_lo = (
                float(last["lo"])
                if last.get("lo") is not None and np.isfinite(last.get("lo"))
                else 50.0
            )
            ...
            prob_bear_t, prob_bull_t = prob_bear, prob_bull
            log.info(
                "  [%s] Bottom 모델 없음 → 폴백 bottom_lo=%.2f%% bottom_day=%d",
                last["symbol"],
                bottom_lo,
                bottom_day,
            )
    pred_is_bull, *_ = judge_bull_bear(
        last,
        grp,
        max_cyc,
        prob_bull,
    ...
    if is_btc_coin:
        peak_hi, peak_day_pred = calc_peak_btc(
            df_all, max_cyc, last, coin_id, cross_median
        )
    else:
        ...
        peak_hi, peak_day_pred = calc_peak_hybrid_for_coin(
            ...
            last,
            cross_median,
            label=str(last["symbol"]).upper(),
        )
    log.debug(
        "    Peak 예측: hi=%.2f%%  day=%s",
        peak_hi if peak_hi else 0.0,
        str(peak_day_pred) if peak_day_pred else "-",
    )
    peak_rows = list(
        _collect_peak_rows(
            coin_id, last, max_cyc, peak_hi, peak_day_pred, bottom_lo, bottom_day
    ...
    return {
        ...
        "phase_box_stats": phase_box_stats,
        ...
        "reg_key": reg_key,
        "pred_hi_bull": pred_hi_bull,
        "pred_lo_bull": pred_lo_bull,
        "pred_dur_bull": pred_dur_bull,
        "is_btc_coin": is_btc_coin,
        "bottom_lo": bottom_lo,
        "bottom_day": bottom_day,
        "prob_bear_t": prob_bear_t,
        "prob_bull_t": prob_bull_t,
        "pred_is_bull": pred_is_bull,
        "peak_hi": peak_hi,
        "peak_day_pred": peak_day_pred,
        "peak_rows": peak_rows,
        "start_x": start_x,
        "ref_lo": ref_lo,
        "cycle_lo": cycle_lo,
...
def _predict_one_coin_phase2(conn: Any, bundle: dict):
    last, coin_id, max_cyc = bundle["last"], bundle["coin_id"], bundle["max_cyc"]
    feat, pred_hi_bull, pred_lo_bull, pred_dur_bull = (
        bundle["feat"],
        bundle["pred_hi_bull"],
        bundle["pred_lo_bull"],
        bundle["pred_dur_bull"],
    )
    pred_is_bull, bottom_lo, bottom_day = (
        bundle["pred_is_bull"],
        bundle["bottom_lo"],
        bundle["bottom_day"],
    )
    start_x, next_box_idx, ref_lo, cycle_lo = (
        bundle["start_x"],
        bundle["next_box_idx"],
        bundle["ref_lo"],
        bundle["cycle_lo"],
    ...
    if cycle_prediction is not None:
        log.info(
            ...
            last["symbol"],
            max_cyc,
            cycle_prediction.bear_count,
            cycle_prediction.bull_count,
            cycle_prediction.method,
            cycle_prediction.guard_applied["bear"],
    ...
    if bottom_lo is not None and bottom_day is not None:
        ...
        if low_info is not None:
            ...
            if not active_rows.empty:
                ...
                bear_cur_day = (
                    int(act["lo_day"])
                    if pd.notna(act.get("lo_day"))
                    else int(act.get("start_x", last["end_x"]))
                )
                bear_cur_val = float(act["lo"])
            else:
                bear_cur_day = (
                    int(last["lo_day"])
                    if pd.notna(last.get("lo_day"))
                    else int(last["end_x"])
                )
                bear_cur_val = (
                    float(last["lo"])
                    if last.get("lo") is not None
                    else float(last["hi"])
                )
            bear_box_start_x = bear_cur_day
        ...
        _bear_2021 = (
            bundle["df_all"][
                ...
                & (
                    bundle["df_all"]["cycle_name"].str.contains(
                        "2021", case=False, na=False
                    )
                )
        ...
        for _i in range(len(_bear_2021) - 1):
            ...
            _ref_declines.append(
                (_next_lo - _curr_hi) / _curr_hi * 100 if _curr_hi > 0 else -30.0
            )
        ...
        log.info(
            "  [%s] BEAR simple_offset=%d (패턴오프셋=%d 무시) ranges=%s declines=%s",
            _sym,
            _simple_offset,
            _bear_offset,
            _ref_ranges_offset,
            _ref_declines_offset,
        )
        ...
        for _i in range(len(_bull_2021) - 1):
            ...
            _ref_bull_pullbacks.append(
                (_next_lo - _curr_hi) / _curr_hi * 100 if _curr_hi > 0 else -10.0
            )
        ...
        log.info(
            "  [%s] BULL offset=%d ranges=%s pullbacks=%s",
            _sym,
            _bull_offset,
        ...
        chain_pred_rows, chain_path_rows = build_bear_chain(
            ...
            bottom_day=bottom_day,
            bottom_lo=bottom_lo,
            cur_day=bear_cur_day,
            cur_val=bear_cur_val,
            feat=feat,
            ...
            max_bear_chain=(
                min(bundle["cycle_prediction"].bear_count, MAX_BEAR_CHAIN)
                if bundle.get("cycle_prediction")
                else None
            ),
        ...
        if chain_path_rows:
            ...
            if (
                peak_hi is not None
                and peak_day_pred is not None
                and peak_day_pred > bottom_day
            ):
                ...
                bull_chain_rows, bull_chain_path = build_bull_chain(
                    ...
                    cycle_lo,
                    max_bull_chain=(
                        min(bundle["cycle_prediction"].bull_count, MAX_BULL_CHAIN)
                        if bundle.get("cycle_prediction")
                        else None
                    ),
                ...
                bull_row_after, _, _ = build_bull_scenario(
                    ...
                    last,
                    max_cyc,
                    next_box_idx + len(chain_pred_rows),
                    bottom_day + 1,
                    ref_lo,
                    cycle_lo,
                    pred_hi_bull,
                ...
                path_rows = (
                    list(chain_path_rows)
                    + [bridge]
                    + [r for r in bull_path_from_bottom if r[6] > bear_end_day]
                )
        else:
            path_rows = list(bull_path_rows)
    sim_symbol, sim_cycle, sim_box, similarity = find_most_similar_pattern(
        bundle["train_df"], bundle["X_pred"]
    )
...
def predict_outputs(
    ...
    cycle_stats, coin_stats, phase_box_stats, btc_cycle_max_hi = (
        build_cycle_and_coin_stats(df_all)
    )
    ...
    return pred_rows, path_rows, peak_rows, pred_count, skip_count
...
def predict_and_insert(
    ...
    deleted = conn.execute(
        "DELETE FROM coin_analysis_results WHERE is_prediction = 1"
    ).rowcount
    conn.commit()
    ...
    _insert_predictions_to_db(
        conn, pred_rows, path_rows, peak_rows, pred_count, skip_count
    )
...
def _insert_predictions_to_db(
    ...
    log.info("=" * 72)
    log.info(
        "  예측 저장 완료: 코인 %d개  스킵 %d개  | pred_rows=%d  path_rows=%d",
        pred_count,
        skip_count,
        len(pred_rows),
        len(path_rows),
...
def print_prediction_summary_rows(rows: list[dict] | list[tuple]):
    if not rows:
...
def print_prediction_summary(conn: Any):
