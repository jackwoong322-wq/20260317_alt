...
type BearBullSeg = { type: 'BEAR' | 'BULL'; startX: number; endX: number; startVal: number; endVal: number; pct: string; days: number };
declare function renderBearBullLabels(segs: BearBullSeg[], ts: any, series: any): void;
declare const LightweightCharts: any;

function getBearBullColors(type: 'BEAR' | 'BULL'): { bgColor: string; lineCol: string } {
  return type === 'BEAR'
    ? { bgColor: 'rgba(255,68,102,0.07)', lineCol: 'rgba(255,68,102,0.0)' }
    : { bgColor: 'rgba(0,255,136,0.07)', lineCol: 'rgba(0,255,136,0.0)' };
}

function getMaxCloseValue(cycleData: any[]): number | null {
  const maxClose = cycleData
    .map((d: any) => d.close)
    .reduce(
      (m: number, v: number) => (Number.isFinite(Number(v)) ? Math.max(m, Number(v)) : m),
      -Infinity,
    );
  // 유효한 close 값이 하나도 없으면 null 반환 → 시리즈 추가 자체를 skip
  if (!Number.isFinite(maxClose)) return null;
  return maxClose * 1.05;
}

function addBearBullSegmentSeries(
  seg: BearBullSeg,
  si: number,
  cycle: any,
  coinId: string,
  ...
  if (maxV === null) return; // 유효한 close 데이터 없음 → skip
  ...
  if (bt1 != null && bt2 != null && Number.isFinite(bt1) && Number.isFinite(bt2)) {
    setSeriesDataSafe(
      bgSeries,
      filterValidPoints([{ time: bt1, value: maxV }, { time: bt2, value: maxV }]),
      { kind: 'bearbull_bg', coinId, symbol: coinData.symbol, cycleName: cycle.cycle_name, cycleNum },
    );
  }
  chartState.seriesMap[`${coinId}_${cycle.cycle_number}_bb${si}`] = bgSeries;
}

export function addBearBullSeries(
  coinId: string,
  coinData: any,
  cycle: any,
  cycleNum: number,
  ...
  if (!chartState.showBearBull || chartState.selectedCoins.length !== 1) return;

  const segs = detectBearBull(cycle.data as any);
  segs.forEach((seg: BearBullSeg, si: number) => {
