 ...
 */
import { chartState, ChartState, COIN_COLORS, CYCLE_COLORS } from './chart-logic.js';
...
function buildCloseKey(coinId: string, cycleNumber: number): string {
  return `${coinId}_${cycleNumber}_close`;
}

/** 단일 시리즈 제거 시도, 실패 시 로그만 남김 (방어적 설계) */
function removeSeriesSafe(chart: any, key: string, series: any): void {
  try {
...
function clearAllSeries(state: ChartState): void {
...
function findCycleMinLowIdx(cycleData: any[]): number {
  let minVal = Infinity;
  let minIdx = 0;
  cycleData.forEach((d: any, idx: number) => {
    if (d.low < minVal) {
      minVal = d.low;
      minIdx = idx;
    }
...
function findBearStartLowIdx(cycleData: any[], zones: any[]): number | null {
  const firstBear = zones?.find((z: any) => z.is_prediction === 1 && z.phase === 'BEAR');
  if (!firstBear || !cycleData?.length) return null;
...
function resolveCycleColor(
  state: ChartState,
  coinIdx: number,
  cycleNum: number,
): string {
  const baseColor = COIN_COLORS[coinIdx % COIN_COLORS.length];
  const col = (CYCLE_COLORS as any)[cycleNum] || (CYCLE_COLORS as any)[1];
  return state.selectedCoins.length > 1 ? baseColor : col.main;
}

/** 순수 함수: 범례 아이템 문자열 (파생 데이터) */
function buildLegendLabel(state: ChartState, coinData: any, cycle: any): string {
...
function drawCycleForCoin(
  ...
  if (!state.activeCycles.has(cycleNum)) return;
  ...
  const lineSeries = addMainLineSeries(
    state, coinId, coinData, cycle, color, isCurr, cycleNum, cycleMinLowIdx, closeKey,
  );
...
export function drawChart(state: ChartState = chartState): void {
  ...
  state.selectedCoins.forEach((coinId: string, coinIdx: number) => {
  ...
  const predictionDump: Record<string, any> = {};
...
(window as any).drawChart = drawChart;
