...
export function setupTooltip(): void {
  ...
  chartState.chart.subscribeCrosshairMove((param: any) => {
    ...
    const rows: any[] = [];
    Object.entries(chartState.seriesMetaMap).forEach(([key, meta]: [string, any]) => {
      ...
      const found = cycle.data.find((d: any) => d.x === dayX);
      const date = found ? found.date : '';
      const col =
        (CYCLE_COLORS as any)[cycle.cycle_number] || (CYCLE_COLORS as any)[1];
      const color =
        chartState.selectedCoins.length > 1
          ? COIN_COLORS[
              chartState.selectedCoins.indexOf(coinId) % COIN_COLORS.length
            ]
          : col.main;
    ...
    rows.forEach((r) => {
      ...
      if (r.boxInfo) {
        ...
        html += `
        <div style="margin:4px 0 2px;padding:6px 8px;background:rgba(${
          isBear ? '255,68,102' : '255,184,0'
        },${isPred ? '0.04' : '0.08'});border:1px solid rgba(${
          isBear ? '255,68,102' : '255,184,0'
        },${isPred ? '0.15' : '0.25'});border-radius:4px;${
          isPred ? 'border-style:dashed;' : ''
        }">
          <div style="font-size:10px;font-weight:700;color:${boxColor};margin-bottom:4px;letter-spacing:1px;">${
          z.phase
        } Box #${z.boxIndex != null ? z.boxIndex + 1 : zi + 1}${
          isPred
          ...
          <div style="display:flex;justify-content:space-between;font-size:10px;margin:2px 0;"><span style="color:#4a6080">현위치</span><span style="color:#fff;">${dayInBox}/${
          z.duration ?? '-'
        }일</span></div>
