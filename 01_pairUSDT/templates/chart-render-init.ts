...
window.addEventListener('error', (e: ErrorEvent) => {
  const msg = e.message ?? (e.error as Error)?.message ?? '';
  if (msg.includes('Value is null')) {
    const count = ((window as any).__nullErrorCount ?? 0) + 1;
    (window as any).__nullErrorCount = count;
...
export function initChart(): void {
  ...
  chartState.chart = LightweightCharts.createChart(el, {
    ...
    localization: {
      timeFormatter: (v: any) => `Day ${v}`,
    },
    timeScale: {
      borderColor: '#1e2d45',
      tickMarkFormatter: (v: any) => `Day ${v}`,
    },
    handleScroll: true,
    handleScale: true,
  });
  ...
  chartState.chart.timeScale().subscribeVisibleTimeRangeChange(() =>
