...
function isValidSubBox(box: any): boolean {
  return (
    box != null &&
...
function addSubBoxBand(
  ...
  setSeriesDataSafe(
    hiLine,
    filterValidPoints([{ time: t1, value: upper }, { time: t2, value: upper }]),
    { kind: isCandidate ? 'subbox_candidate_hi' : 'subbox_hi', ...meta },
  );
  ...
  setSeriesDataSafe(
    loLine,
    filterValidPoints([{ time: t1, value: lower }, { time: t2, value: lower }]),
    { kind: isCandidate ? 'subbox_candidate_lo' : 'subbox_lo', ...meta },
  );
  ...
  setSeriesDataSafe(
    fill,
    filterValidPoints([{ time: t1, value: upper }, { time: t2, value: upper }]),
    { kind: isCandidate ? 'subbox_candidate_fill' : 'subbox_fill', ...meta },
  );
  chartState.seriesMap[`${keyPrefix}_sub_fill`] = fill;
}

export function addSubBoxSeries(
  coinId: string,
  coinData: any,
  cycle: any,
  cycleNum: number,
  ...
  if (!chartState.showSubBox || !lineSeries || !chartState.chart) return;
  ...
  const candidates = Array.isArray(cycle.sub_box_candidates)
    ...
    : [];
  if (subBoxes.length === 0 && candidates.length === 0) return;
  ...
  subBoxes.forEach((box: any, idx: number) => {
    addSubBoxBand(box, `${coinId}_${cycle.cycle_number}_sub_${idx}`, false, meta);
  });
  candidates.forEach((box: any, idx: number) => {
