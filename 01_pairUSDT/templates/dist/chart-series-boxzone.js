...
function addBoxHiLoLines(z, zi, isPred, lineColor, coinId, coinData, cycle, cycleNum) {
...
function addBoxFillSeries(z, zi, fillTop, fillBot, coinId, coinData, cycle, cycleNum) {
    const meta = { coinId, symbol: coinData.symbol, cycleName: cycle.cycle_name, cycleNum };
    const t1 = dayToTime(z.startX);
    const t2 = dayToTime(z.endX);
...
function addActiveBoxExtensionForZone(z, coinId, coinData, cycle, cycleNum, lastRealDay, lastRealClose) {
    ...
    if (actBear) {
        if (hiDay > lastRealDay)
            pts.push({ time: hiDay, value: z.hi });
        if (loDay > lastRealDay && loDay > (pts[pts.length - 1]?.time ?? 0))
            pts.push({ time: loDay, value: z.lo });
    }
    else {
        if (loDay > lastRealDay)
            pts.push({ time: loDay, value: z.lo });
        if (hiDay > lastRealDay && hiDay > (pts[pts.length - 1]?.time ?? 0))
            pts.push({ time: hiDay, value: z.hi });
    }
    const tsPts = pts.map((p) => { const ts = dayToTime(p.time); if (ts == null || p.value == null)
        return null; return { time: ts, value: p.value }; }).filter((p) => p !== null);
    if (tsPts.length <= 1)
        return;
...
export function addBoxZoneSeries(coinId, coinData, cycle, cycleNum, cycleMinLowIdx, closeKey, lineSeries) {
    if (!lineSeries) {
...
export function addBoxZoneFallback(coinId, coinData, cycle, cycleMinLowIdx, closeKey, lineSeries) {
    if (!lineSeries) {
...
export function addActiveBoxExtensions(coinId, coinData, cycle, cycleNum) {
    if (!cycle.box_zones)
        return;
