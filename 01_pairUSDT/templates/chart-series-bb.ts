 ...
 */
export function calculateBollingerBands(data: any[], period = 20, stdDevMultiplier = 2) {
  ...
  for (let i = period - 1; i < data.length; i++) {
    ...
    for (let j = 0; j < period; j++) {
      ...
      if (!Number.isFinite(val)) continue; // 안전망
      sum += val;
    }
 ...
 */
export function addBollingerBandsSeries(
  state: ChartState,
  coinId: string, symbol: string, cycleName: string, cycleNum: number, cycleData: any[],
  closeKey: string
): void {
  if (!cycleData || cycleData.length < 20 || !state.chart) return;
