...
from lib.common.coingecko_ohlcv import fetch_recent_daily as coingecko_fetch_recent_daily
...
def binance_fetch_usdt_symbols() -> set[str]:
    ...
    if not data:
        raise RuntimeError("Binance exchangeInfo 조회 실패 (모든 호스트)")
...
def main():
    ...
    for i, coin in enumerate(coins, 1):
        ...
        if klines:
            ...
            log.warning(
                "  Binance kline 없음 → CoinGecko market_chart (최근 365일, 무료 티어 상한)"
            )
            rows = coingecko_fetch_recent_daily(coin_id, 365)
        ...
        if rows:
            ...
            log.info(f"  [{source_label}] 요청 {saved}건 / 배치 {batch_count}")
            log.info(f"  [{source_label}] 실제 반영 {reflected}건 (count delta)")
            log.info(f"  [{source_label}] 최초 일자: {earliest}")
        else:
            log.warning(f"  Binance/CoinGecko 일봉 없음 → 2단계에서 전체 수집")
            earliest = None
